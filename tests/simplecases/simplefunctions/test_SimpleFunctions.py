...
class SimpleFunctionsTest(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        os.chdir("../../..")
        os.system(r'python cBinder sources -v -f ' + str(self.sources_path) + ' -d ' + str(self.destination_path) + ' compile')
        os.chdir(self.destination_path)

    def test_generate_bindings_to_simple_add_function_integer(self):
        from tests.simplecases.simplefunctions.generated.sources import add
        sum = add.add(1, 2)
    ...
    def test_generate_bindings_to_simple_print_char_function(self):
        from tests.simplecases.simplefunctions.generated.sources import char
